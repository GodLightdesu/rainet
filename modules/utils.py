...
def scoreBoard(game):
    ...
    # if winner is not None and winner == game.player.name: return 100
    # elif winner is not None and winner != game.enemy.name: return -100
    enemyPieces = len(game.board.getAllyPieces(game.enemy.color))
    allyPieces = len(game.board.getAllyPieces(game.player.color))
    if enemyPieces == 0: return 100
    elif allyPieces == 0: return -100
...
def scoreMaterial(game):
    ...
    # if enemyLink == 0 and game.player.link_eat == 4: return 1000
    
    if game.enemy.virus_eat == 4: return 1000
    elif game.enemy.link_eat == 4: return -1000
    ...
    score += PIECEVALUE['unknown'] * (game.player.link_eat + game.player.virus_eat)
    
    # pieces in server
    # print(game.enemy.name + ' serverStack:', end=' ')
    for piece in game.enemy.serverStack:
      # print(piece.name, end=' ')
      if piece.name == 'link': score += PIECEVALUE['link'] * 2
      elif piece.name == 'virus': score -= PIECEVALUE['virus'] * 3
    # print()
    
    # print(game.player.name + ' serverStack:', end=' ')
    for piece in game.player.serverStack:
      # print(piece.name, end=' ')
      if piece.checked and piece.name == 'virus': score += PIECEVALUE['virus'] * 3
    ...
    for piece in allyPieces:
      if piece.name == 'link': 
        ...
        score += 10 - distanceToExit(game, game.player.color, startRow, startCol)
    
    for piece in enemyPieces:
      startRow, startCol = game.board.findPiecePos(piece)
      if game.view == 'blue': startRow = BROW[startRow]
      score -= 10 - distanceToExit(game, game.enemy.color, startRow, startCol)
